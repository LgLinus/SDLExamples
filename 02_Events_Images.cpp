...
int main(int argc, char* args[]){
	...
	// APply current image
	SDL_BlitSurface(gCurrentSurface, NULL, gScreenSurface, &locationImage);
	// UPdate surface
...
bool loadMedia(){
	...
	gKeyPressSurfaces[KEY_PRESS_SURFACE_DEFAULT] = SDL_LoadBMP("gfx/lesson4/default.bmp");
	if (gKeyPressSurfaces[KEY_PRESS_SURFACE_DEFAULT] == NULL){
		printf("Failed to load default image!\n");
		success = false;
	}
	gKeyPressSurfaces[KEY_PRESS_SURFACE_DOWN] = SDL_LoadBMP("gfx/lesson4/down.bmp");
	if (gKeyPressSurfaces[KEY_PRESS_SURFACE_DOWN == NULL]){
		printf("failed to load down image\n");
		success = false;
	}
	gKeyPressSurfaces[KEY_PRESS_SURFACE_UP] = SDL_LoadBMP("gfx/lesson4/up.bmp");
	if (gKeyPressSurfaces[KEY_PRESS_SURFACE_UP == NULL]){
		printf("failed to load up image\n");
		success = false;
	}
	gKeyPressSurfaces[KEY_PRESS_SURFACE_RIGHT] = SDL_LoadBMP("gfx/lesson4/right.bmp");
	if (gKeyPressSurfaces[KEY_PRESS_SURFACE_RIGHT == NULL]){
		printf("failed to load right image\n");
		success = false;
	}
	gKeyPressSurfaces[KEY_PRESS_SURFACE_LEFT] = SDL_LoadBMP("gfx/lesson4/left.bmp");
